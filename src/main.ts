...
async function run(): Promise<void> {
  try {
    ...
    const releaseAssets = core.getMultilineInput('releaseAssets');
    ...
    core.debug(`releaseAssets=${releaseAssets}`);
    ...
    if (!versionStr && releaseAssets.length === 0) {
      throw new Error(
        "must specify either the 'version' parameter OR 'releaseAssets' parameters."
      );
    }

    if (versionStr && releaseAssets.length > 0) {
      core.warning(
        "'version' parameter specified as well as 'releaseAssets' parameter; using 'version' parameter only"
      );
    }
    ...
    if (versionStr) {
      ...
      const nameRegex = new RegExp(releaseAssets[0]);
      const asset = assets.find(x => nameRegex.test(x.name));
      if (!asset) {
        throw new Error(
          `unable to find an asset matching '${releaseAssets[0]}' in repo '${releaseRepo}'`
        );
      }
